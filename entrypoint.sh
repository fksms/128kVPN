...
if ${BANDWIDTH_LIMIT}; then
    ...
    for i in $(seq 2 254); do
        ...
        tc class add dev ${WG_INTERFACE_NAME} parent 1: classid 1:$i htb rate ${RATE} ceil ${RATE}
        tc filter add dev ${WG_INTERFACE_NAME} protocol ip parent 1: prio 1 u32 match ip dst $ip flowid 1:$i

        # Per-client ingress limit classes and filters
        tc class add dev ${IFB_INTERFACE_NAME} parent 1: classid 1:$i htb rate ${RATE} ceil ${RATE}
        tc filter add dev ${IFB_INTERFACE_NAME} protocol ip parent 1: prio 1 u32 match ip src $ip flowid 1:$i
    done
