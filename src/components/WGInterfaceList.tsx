        ...
        <div className="space-y-4">
            ...
            <dialog id={CreateInterfaceModalId} className="modal">
                <div className="modal-box w-96 max-w-xl">
                    ...
                    <input type="text" value={createWGInterfaceName} onChange={e => setCreateWGInterfaceName(e.target.value)} placeholder={t("WGInterfaceList.CreateModal.placeholder")} className="input" />
                    <p className="text-sm text-error mt-2">{createWGInterfaceError}</p>

                    <div className="flex justify-end space-x-2 mt-4">
                        <button className="btn btn-soft btn-primary" onClick={() => { createWGInterface() }}>{t("WGInterfaceList.CreateModal.submit")}</button>
                        <button className="btn" onClick={() => {
                            closeModal(CreateInterfaceModalId);
                            setCreateWGInterfaceError("");
                        }}>{t("WGInterfaceList.CreateModal.cancel")}</button>
                    </div>
                </div>
            </dialog>
            {/*--------------------インターフェース作成モーダル--------------------*/}
            ...
            <dialog id={DeleteInterfaceModalId} className="modal">
                <div className="modal-box w-96 max-w-xl">
                    ...
                    <div className="flex justify-end space-x-2">
                        <button className="btn btn-soft btn-primary" onClick={() => { deleteWGInterface() }}>{t("WGInterfaceList.DeleteModal.submit")}</button>
                        <button className="btn" onClick={() => { closeModal(DeleteInterfaceModalId) }}>{t("WGInterfaceList.DeleteModal.cancel")}</button>
                    </div>
                </div>
            </dialog>
            {/*--------------------インターフェース削除モーダル--------------------*/}
